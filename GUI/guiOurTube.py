...
class MainApplication:
    def __init__(self, master):
        ...
        # To make the window of constant size
        ...
        self.topframe = ttk.Frame(
            self.master, relief=GROOVE, height=110, width=580)
        self.topframe.place(x=10, y=30)

        # YouTube URL label
        self.label1 = Label(
            self.topframe, text='Enter the URL of the YouTube video ', font=("Agency FB", 20))
        self.label1.place(x=110, y=5)

        # To get the URL of the video through entry box
        self.Link = ttk.Entry(self.topframe, width=80)
        self.Link.place(x=40, y=50)

        # Entry error label if user inputs incorrect
        self.youtubeEntryError = Label(
            self.master, text="", font=("Comic Sans MS", 11))
        self.youtubeEntryError.place(x=150, y=103)

        # Asking where to save file label
        self.youtubeFileLocationLabel = ttk.Label(
            self.master, text="Please choose folder where to save file : ", font=("Agency FB", 20))
        self.youtubeFileLocationLabel.place(x=25, y=150)

        # Button to open directory
        self.youtubeFileLocationEntry = ttk.Button(
            self.master, text="Select Folder", command=self.SelectFolder)
        ...
        self.youtubeChooseLabel = ttk.Label(
            self.master, text="Please choose Type of File : ", font=("Agency FB", 20))
        ...
        for text, mode in downloadChoices:
            if(i == 1):
                self.youtubeChoices = ttk.Radiobutton(
                    self.master, text=text, variable=self.ChoicesVar, value=mode).place(x=260, y=220)
                i = i+1
            else:
                self.youtubeChoices = ttk.Radiobutton(
                    self.master, text=text, variable=self.ChoicesVar, value=mode).place(x=360, y=220)

        self.download_btn = ttk.Button(
            self.master, text="Download", command=self.checkYoutubeLink)
        ...
        self.middleframe = ttk.Frame(
            self.master, relief=GROOVE, height=120, width=580)
        ...
        self.label3 = ttk.Label(
            self.middleframe, text="Download YouTube content for free to watch offline: videos,playlist.   Download heavy 4k, Full HD,HD Files fast & easily. Video Downloader will show you size before saving it.", wraplength=400)
    ...
    def SelectFolder(self):
        ...
        if(len(self.FolderName) > 1):
            self.fileLocationLabelError.config(
                text=self.FolderName, fg="green")
            return True
        else:
            self.fileLocationLabelError.config(
                text="Please choose folder!", fg="red")

    # To Check Whether URL is Valid and Throw and Error if it isn't
    def checkYoutubeLink(self):
        matchYoutubeLink = re.match(
            '^https://www.youtube.com/.*', self.Link.get())
        self.URL = self.geturl()
        if(self.URL == ''):
            self.youtubeEntryError.config(
                text="Please Enter Video URL", fg="red")
        elif(not matchYoutubeLink):
            self.youtubeEntryError.config(
                text="Please Enter valid Youtube URL", fg="red")
        ...
        try:
            ...
            if(self.ChoicesVar.get() == 1):
                with youtube_dl.YoutubeDL(ydl_opts) as ydl:
                    self.downloading_status.config(
                        text="file is downloading...", fg="green")
                    ydl.download([URL])
                    self.downloading_status.config(
                        text="file is downloaded", fg="green")
                    check = true
            else:
                ydl_opts = {
            ...
            if check == True:
                ...
                self.downloading_status.config(
                    text='Sorry,something went wrong!', fg="red")
